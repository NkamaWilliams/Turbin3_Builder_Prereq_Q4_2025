...
use crate::{errors::TruthLieError, state::{CollectionAuthority, WhitelistedCreators}};
...
pub struct CreateCollection<'info> {
    #[account(
        mut,
        constraint = whitelisted_creators.contains(&creator) @TruthLieError::NotAuthorized
    )]
    pub creator: Signer<'info>,
    #[account(
        mut,
        constraint = collection.data_is_empty() @TruthLieError::CollectionAlreadyInitialized
    )]
