...
import { MPL_CORE_PROGRAM_ID, isFrozen, fetchAsset, fetchCollectionV1, mplCore } from "@metaplex-foundation/mpl-core";
  ...
  describe("FreezeNft", () => {
    it("Freeze an NFT", async () => {
      try {
        await program.methods
          ...
          assert.isTrue(await isFrozen(assetv1, collectionv1));
        } catch (e) {
        const assetv1 = await fetchAsset(umi, asset.publicKey.toString());
        console.error("Freeze 1 error:", e);
        console.log("AssetV1", assetv1);
        assert.fail("Freeze NFT Failed");
      }
    });
    ...
    it("Fails to freeze with unauthorized authority", async () => {
      try {
        ...
        // console.error("Freeze 2 Error:", err)
